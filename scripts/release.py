"""
release.py — prepare an eyerate release commit.

What it does:
  1. Verifies eyerate VERSION is currently <target>_dev.
  2. Writes the clean version to eyerate VERSION.
  3. Calls sync_version.sync() to propagate eyerate version and matika_version
     (read from sibling matika clone / MATIKA_VERSION env var) into applug.json.
  4. Runs drift_check() — applug.json must exactly match both versions.
...
from sync_version import REPO_ROOT, drift_check, read_matika_version, read_version, sync  # noqa: E402
...
def main() -> None:
    ...
    print(f"Releasing eyerate {target_clean}  (matika_version={matika_version!r})")
    ...
    # 4. Drift check
    drift_check(target_clean, matika_version)
