...
def main():
    ...
    parser.add_argument("--version", required=True, help="Version to release (e.g., v0.0.2)")
    ...
    print("\n" + "="*50)
    print("   EYERATE RELEASE: ARCHITECT PLAN OF ACTION")
    print("="*50)
    ...
    print(f"6. Synchronize version in applug.json")
    print(f"7. gh release create {version} --generate-notes")
    ...
    # 4. Sync Version
    print(f"[4/6] Synchronizing version from VERSION file...")
    run(f"{sys.executable} scripts/sync_version.py")

    # 5. Release (Plugins might not have dist wheels, just tagging the source)
    print(f"[5/6] Creating GitHub Release {version}...")
    run(f"gh release create {version} --generate-notes")
