...
- **L2 — generic structural harness.** Domain-blind, manifest-driven. tier-a asserts every declared screen's route is alive, authorized, and renders HTML over authenticated HTTP; tier-b drives each declared screen's steps through a headless browser (Playwright) via a generic verb executor and asserts markers in the live DOM. matika owns the contract; the ahimsa gate runs it. eyerate's role is to DECLARE its screens, markers, and steps in `src/eyerate/eyerate_screens.json`; it does not own the harness.
- **L3 — applug-authored functional tests, generically invoked by the gate, reboot-per-applug.** eyerate authors its functional tests (`src/eyerate/eyerate_functional_tests.json` + `eyerate_functional_tests.py`, functional-test schema version 1.0); the product gate (ahimsa) discovers them from a SHA-pinned source clone, boots a fresh app in a clean HOME, runs eyerate's tests in randomized (seeded) order, and tears down. Who authors (the applug) is separate from who invokes (the generic gate); there is no isolation requirement, and reboot is coarse containment between applugs only (no within-applug reboot). Each test self-arranges its server-side provider precondition and self-resets to the default (`yahoo`) provider via a guaranteed-run `try/finally` — known-initial-state in, known-initial-state out — so reset holds even when the body assertion fails; randomized order is how the gate verifies that reset discipline holds. A test that cannot reset its own mutation is a defect, never rebooted-around. eyerate is the canonical self-arrange/self-reset exemplar; its declared tests are `eyerate:lookup_voo`, `eyerate:search_voo`, and `eyerate:keyless_finnhub_502`. Merged to `main` via `manomatika/eyerate#63`.
...
- `applug.json` — Plugin `id` (`eyerate`), `name` (`"EyeRate"`), `description`, `version`, `matika_version`, entry point, permissions, and the `settings_ui` schema. There is **no** `display_name` key (the UI label falls back to `name`). The `settings_ui` schema drives the data-provider selector on Matika's settings page (section `data_providers`): a `financial_security_data_endpoint` select (`yahoo`/`finnhub`/`alphavantage`, default `yahoo`) and a `financial_security_data_api_key` textbox shown only when the endpoint is `finnhub` or `alphavantage`. Permissions (both `Maintenance` page type): `/eyerate/securities` — Admin=FULL, User=FULL; `/eyerate/admin` — Admin=FULL, User=NONE.
